...
test('can pass turns without playing a card', async ({browser}) => {
	test.slow()
	...
	// can see the top discard card
	const discardPileRegion1 = page1.getByRole('list', {name: 'Discard Pile'})
	await expect(discardPileRegion1.getByRole('img')).toBeVisible()
	...
	// Player 1 immediately ends turn, villain draws a card!
	await page1.getByRole('button', {name: 'End Turn'}).click()
	await expect(page1.getByRole('button', {name: 'End Turn'})).toBeDisabled()

	// Player 2 immediately ends turn, Player 1 draws a card!
	await page2.getByRole('button', {name: 'End Turn'}).click()
	await expect(page2.getByRole('button', {name: 'End Turn'})).toBeDisabled()

	// Player 1 should now have 4 + 1 cards
	await expect(heroHandRegion1.getByRole('listitem')).toHaveCount(5)
	// Player 1 could still end their turn if they wish
	await expect(page1.getByRole('button', {name: 'End Turn'})).toBeEnabled()
